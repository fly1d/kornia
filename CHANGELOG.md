...
* Check the supported `kornia.morphology` half-precision row on every pull request and add a nightly ratchet for
  ...
  failing tests, and lists tests that have started passing so the baseline can be tightened deliberately. Test groups
  run in fresh processes to keep resource-heavy half-precision tests isolated (#4070).
